...
This version combines rewards and states
"""
...
def construct_sequence_with_goal_and_plan(goal, plan, sa_sequence):
    first_s = sa_sequence[:, :1]  # shape [batch_size, 2, emb_dim (or 1 if mask)]
    remaining_elements = sa_sequence[:, 1:]  # shape [batch_size, 3*seq_len-2, emb_dim (or 1 if mask)]
    return torch.cat([goal, first_s, plan, remaining_elements], dim=1)
...
class PlanningDecisionTransformer(DecisionTransformer):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList(
            [
                TransformerBlock(
                    seq_len=2 * seq_len + plan_length + 1,  # Adjusted for the planning token and goal
                    embedding_dim=embedding_dim,
                    num_heads=num_heads,
                    attention_dropout=attention_dropout,
                    residual_dropout=residual_dropout,
        ...
        self.state_emb = nn.Linear(state_dim+1, embedding_dim) # add 1 to state dim to account for concatenated reward
    ...
    def forward(self, goal, states, actions, returns_to_go, time_steps, plan, padding_mask=None,
        ...
        states = torch.cat([states,returns_to_go.unsqueeze(-1)],dim=-1)
        ...
        # convert to form (goal, s_0, p_0, p1, ..., p_n, a_0, s_1, a_1, ...)
        ...
        out_plan = self.planning_head(out[:, 1: 1 + self.plan_length])

        # predict actions only from state embeddings
        out_states = torch.cat([out[:, 1:2], out[:, (3 + self.plan_length)::2]], dim=1)
