...
class TrainConfig:
    ...
    def __post_init__(self):
        ...
        self.plan_length = self.num_plan_points # force this as we're making each plan point be a plan token
...
class SequenceManualPlanDataset(SequenceDataset):
    ...
    def create_plan(self, states):
        if self.plan_length:
            path = np.array(simplify_path_to_target_points_by_distance(states[:, :2], self.path_length))
            # path = np.array(simplify_path_to_target_points_by_distance_log_scale(states, self.path_length))
            plan = pad_along_axis(path, pad_to=self.plan_length, axis=0)
            return plan
        else:
            return np.empty((self.plan_length, states.shape[1]))  # Adjusted to match the new shape expectations.
...
class PlanningDecisionTransformer(DecisionTransformer):
    ...
    def forward(self, goal, states, actions, returns_to_go, time_steps, plan, padding_mask=None,
        ...
        # state_repeated =  states[:, -1:, :].clone().repeat(1,plan.shape[1],1).detach()
        ...
        plan_emb = self.plan_emb(plan) + plan_pos_emb\
            if self.plan_length else torch.empty(batch_size, 0, self.embedding_dim, device=device)
...
def train(config: TrainConfig):
    ...
    for step in trange(step_start, config.update_steps+step_start, desc="Training"):
        ...
        plan_weight = np.exp(-2.3e-05*step)
        combined_loss = (1-plan_weight) * action_loss + plan_weight * plan_loss if config.plan_length else action_loss
        # Backpropagation and optimization for main model

        plan_pos_loss = F.mse_loss(predicted_plan[:, :, :2], plan[:, :, :2].detach(),reduction="mean")
            ...
            for batch_index in range(10):
                ...
                if config.plan_length:
                    ...
                    for i, path in enumerate([predicted_plan, plan]):
                        transformed_path = path[batch_index, :, :2].detach().cpu()
        ...
        wandb.log(
            {
                ...
                "train_plan_poss_loss": plan_pos_loss.item(),
